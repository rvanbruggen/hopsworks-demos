...
def generate_historical_day(date: date, start_date: date, data_list: List[Tuple[date, int, float]]) -> List[Tuple[date, int, float]]:
    """
    Generates synthetic data for a given day with different coffeevolume patterns for each ID.
    ...
    # Calculate smoother coffeevolume variations with increased range and seasonality
    days_since_start = (date - start_date).days
    coffeevolume_base = 200 + 30 * np.sin(2 * np.pi * days_since_start / 365)
    coffeevolume_variation = (
        10 * np.sin(2 * np.pi * days_since_start / 30) +
        5 * np.sin(2 * np.pi * days_since_start / 7)  # Weekly pattern
    )

    # Generate a range of coffeevolumes based on the calculated variations
    coffeevolumes = np.linspace(coffeevolume_base - coffeevolume_variation, coffeevolume_base + coffeevolume_variation, num_entries)

    for _ in range(num_entries):
        # Randomly select an ID from the list of IDs
        selected_id = np.random.choice(ids)
        # Ensure non-negative coffeevolumes
        coffeevolume = max(coffeevolumes[_], 0)
        # Append the generated data entry to the data list
        data_list.append((date, selected_id, round(coffeevolume, 1)))
...
def generate_historical_data(start_date: Optional[date] = None, end_date: Optional[date] = None) -> pd.DataFrame:
    """
    Generates synthetic data for a range of dates with different coffeevolume patterns for each ID.
    ...
    Returns:
    - pd.DataFrame: A DataFrame containing the generated data with columns ['date', 'id', 'coffeevolume'].
    """
    ...
    df = pd.DataFrame(data_list, columns=['date', 'id', 'coffeevolume'])
...
def generate_today() -> pd.DataFrame:
    ...
    Returns:
        pandas.DataFrame: A DataFrame containing data for the current date, including date,
                          selected ID, and a random coffeevolume value.
    """
    num_entries = 5000  # 5000 rows per day
    ids = np.arange(5001)  # IDs from 0 to 5000
    data_list = []
    ...
    for entry in range(num_entries):
        selected_id = np.random.choice(ids)
        data_list.append((date, selected_id, round(coffeevolumes[entry], 1)))
...
def to_wide_format(data: pd.DataFrame) -> pd.DataFrame:
    ...
    Parameters:
    - data (pd.DataFrame): The input DataFrame containing time series data with columns ['date', 'id', 'coffeevolume'].

    Returns:
    - pd.DataFrame: A DataFrame in wide format with 'date' as the index, 'id' as columns, and 'coffeevolume' values.
    """
    # Convert the 'date' column to datetime type
    data['date'] = pd.to_datetime(data['date'], format='%Y-%m-%d')

    # Aggregate duplicate entries by taking the mean of coffeevolumes
    agg_df = data.groupby(['date', 'id'])['coffeevolume'].mean().reset_index()

    # Pivot the aggregated DataFrame into wide format
    pivoted_df = agg_df.pivot_table(index='date', columns='id', values='coffeevolume', fill_value=None)
...
def plot_historical_id(ids_to_show: List[int], data: pd.DataFrame) -> go.Figure:
    ...
    - data (pd.DataFrame): The DataFrame containing the data to be plotted, with columns ['date', 'id', 'coffeevolume'].
    ...
    fig = px.line(
        ...
        y='coffeevolume', 
        color='id',
        title=f'Historical coffeevolumes for {ids_to_show} IDs',
        labels={'date': 'Date', 'coffeevolume': 'coffeevolume'},
        line_group='id',
...
def plot_prediction_test(
    ...
    if predictions is not None:
        ...
        pred_df['coffeevolume'] = predictions
        fig.add_trace(go.Scatter(
            x=pred_df['date'], 
            y=pred_df['coffeevolume'],
            mode='lines',
            name='Prediction', 
            line=dict(color='red')
        ))
...
def plot_prediction(
    ...
    """
    Display a time series plot for a specific ID, showcasing historical data, real coffeevolumes, and predicted coffeevolumes.
    ...
    - predictions (pd.Series or None, optional): Predicted coffeevolume values for the test data. Default is None.
    ...
    fig.add_trace(go.Scatter(
        ...
        y=data_historical['coffeevolume'],
        mode='lines',
    ...
    fig.add_trace(go.Scatter(
        ...
        y=data_last_week['coffeevolume'],
        mode='lines',
        name='Real coffeevolume', 
        line=dict(color='green')
    ))
    ...
    # Customize plot layout
    fig.update_layout(
        title=f'Predicted coffeevolume for the {id_to_show} ID',
        xaxis_title='Date',
        yaxis_title='coffeevolume',
        legend_title='Data Type'
    )
